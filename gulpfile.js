  ...
  return gutil.buffer(function(err, files) {
    files
      .filter(function(file) {
        return file.path.endsWith(".capnp");
      })
      .forEach(function(file) {
        gutil.log(
          "Compiling",
          "'" + gutil.colors.cyan(path.relative(__dirname, file.path)) + "'..."
        );
        var includeDir = path.resolve(
          path.join(__dirname, "packages", "capnp-ts", "src", "std")
        );
        var dirname = path.resolve(path.dirname(file.path));
        var options = ["-o./packages/capnpc-ts/bin/capnpc-ts.js"];
        if (includeDir !== dirname) options.push("-I", includeDir);
...
gulp.task("lint", function() {
  ...
  return gulp.src("packages/*/src/**/*.ts").pipe(tslint({ program }));
});
