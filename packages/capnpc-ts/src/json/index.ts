...
const lookupNode = R.curry(
  (ctx: GenContext, req: INode_NestedNode | string): INode => {
...
const generateHeaderComment = R.curry(
  (ctx: FileGenContext): void => {
    const { fd } = ctx;
...
const generateImports = R.curry(
  (ctx: FileGenContext): void => {
    ...
    const importPath =
      (importAnnotation && importAnnotation.value.text) || "capnp-ts";
...
const generateUnnamedUnionEnum = R.curry(
  (ctx: FileGenContext, node: INode): void => {
...
const toJsType = R.curry(
  (ctx: FileGenContext, type: IType, constructor: boolean): string => {
    ...
    switch (whichType) {
      ...
      case Type.LIST:
        ...
        return `capnp.List${constructor ? "Ctor" : ""}<${toJsType(
          ctx,
...
const toConcreteListType = R.curry(
  (ctx: FileGenContext, type: IType): string => {
    ...
    if (elementType === Type.LIST) {
      return `capnp.PointerList(${toConcreteListType(
        ctx,
        type.list.elementType
      )})`;
      ...
      if (structNode.struct.preferredListEncoding !== "inlineComposite") {
        throw new Error(
          `don't know how to make non-inline struct lists with: ${structNode}`
        );
      }

      return `capnp.CompositeList(${toJsType(
        ctx,
        type.list.elementType,
        false
      )})`;
...
const generateStructField = R.curry(
  (ctx: FileGenContext, node: INode, field: IField): void => {
    if (node.struct === undefined) throw new Error("wat");
    ...
    if (field.slot) {
      ...
      throw new Error(
        `don't know how to generate struct field: ${JSON.stringify(field)}`
      );
    }
    ...
    switch (whichType) {
      ...
      case Type.LIST:
        if (field.slot === undefined || field.slot.type.list === undefined) {
          throw new Error("wat");
        }
        ...
        if (
          whichElementType === Type.LIST ||
          whichElementType === Type.STRUCT
        ) {
      ...
      case Type.STRUCT:
        ...
        adopt = true;
        disown = true;
        get = `this._getStruct(${offset}, ${toJsType(
          ctx,
          field.slot.type,
          false
        )})`;
        has = true;
        init = `this._initStructAt(${offset}, ${toJsType(
          ctx,
          field.slot.type,
          false
        )})`;
    ...
    fs.writeSync(
      ...
      `${
        adopt
          ? `
  adopt${Name}(value: capnp.Orphan<${JsType}>): void {${
              union
                ? `
...
`
  ...
  get${Name}(): ${JsType} {${
    ...
    this._testWhich('${name}', this._getUint16(${discriminantOffset *
                    2}), ${discriminantValue});`
                : ""
            }
...
`
        ...
        init
          ? `
  init${Name}(${
              whichType === Type.DATA || whichType === Type.LIST
                ? "length: number"
                : ""
            }): ${JsType} {${
              union
...
const generateStructNode = R.curry(
  (ctx: FileGenContext, node: INode): void => {
    if (node.struct === undefined) throw new Error("wat");
...
export class ${Class} extends capnp.Struct {
  ...
  static readonly _size: capnp.ObjectSize = new capnp.ObjectSize(${dataWordCount *
    8}, ${pointerCount});
`
    );
...
${R.map(
          field =>
            `  static readonly ${c2s(field.name)} = ${Class}_Which.${c2s(
              field.name
            )};`,
          unionFields
        ).join("\n")}
...
const generateEnumNode = R.curry(
  (ctx: FileGenContext, node: INode): void => {
...
const generateNode = R.curry(
  (ctx: FileGenContext, node: INode): void => {
    ...
    switch (whichNode) {
      ...
      default:
        throw new Error(
          `Don't know how to generate a "${Node_Which[whichNode]}" node.`
        );
    }
...
const generateFile = R.curry(
  (
    ...
    try {
      ...
      const fileState: FileGenContext = {
        ...ctx,
        concreteListInitializers,
        fd,
        fileNode,
        requestedFile
      };
