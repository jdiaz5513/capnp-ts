...
tap.test('createValueExpression', (t) => {
  ...
  const node = m.getRoot(s.CodeGeneratorRequest)
    .getNodes().find((n) => n.getDisplayName().split(':')[1] === 'TestDefaults');
  ...
  t.equal(
    printer.printNode(ts.EmitHint.Expression, createValueExpression(value), sourceFile),
